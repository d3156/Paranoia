...
PacketStore::PacketStore(const std::string& db_path) {
    ...
    if (!status.ok()) {
        throw std::runtime_error("Cannot open RocksDB: " + status.ToString());
    }
}
...
// Формируем ключ вида "user123:000000000000000042"
...
std::vector<std::pair<uint64_t, std::vector<uint8_t>>> PacketStore::pull(
        const std::string& user_id, uint64_t after_seq) {
